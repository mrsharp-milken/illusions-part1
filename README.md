...
small bug — and your job is to fix it.
...
| `dudraw.filled_rectangle(x, y, half_w, half_h)` | Filled rectangle centered at `(x, y)`. Note the last two args are *half*-width/height. |
...
## A Useful Idea for All Three Loop Warm-Ups
...
Before writing or fixing any drawing code, it can help to just print
what your formula produces:
...
Look at the numbers before you look at the picture. If the numbers
aren't doing what you expect, the drawing won't either — and it's a
lot easier to spot a bad formula in a column of numbers than in a
window full of circles.
...
- `new_random_color()` is provided for you (don't change it) — but
  look at *where* it's being called.
- Once colors are fixed, look at how `x` is computed for rectangle
  `i`. It should land in a different spot each time.
...
the color gets darker. Look closely at the 4 circles already there —
the radius math (`MAX_RADIUS - _ * RING_STEP`) is already spelled out
for you across the 4 calls, and so is the brightness (dropping by 35
each time). Your job is to turn that pattern into one loop, using `i`
in place of the `0`, `1`, `2`, `3` that are hardcoded right now.
